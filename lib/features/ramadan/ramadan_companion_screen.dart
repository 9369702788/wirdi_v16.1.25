...
class _RamadanCompanionScreenState extends State<RamadanCompanionScreen> {
  ...
  bool _taraweehToday = false;
  int _taraweehDaysLogged = 0;
  ...
  Future<void> _loadFastingState() async {
    ...
    if (hijri.isRamadan) {
      ...
      var daysBack2 = 0;
      while (daysBack2 < 30) {
        final h2 = HijriDate.fromGregorian(cursor2);
        if (!h2.isRamadan) break;
        cursor2 = cursor2.subtract(const Duration(days: 1));
        daysBack2++;
      }
      final startOfMonth2 = now.subtract(Duration(days: daysBack2 - 1));
      taraweehCount = await UserProgressService.taraweehDaysInRange(startOfMonth2, now);
    }
    if (mounted) {
      setState(() {
        _fastingToday = fasted;
        _ramadanDaysLogged = loggedCount;
        _taraweehToday = prayedTaraweeh;
        _taraweehDaysLogged = taraweehCount;
      });
    }
  }
  ...
  Future<void> _toggleTaraweeh() async {
    final next = !_taraweehToday;
    setState(() => _taraweehToday = next);
    await UserProgressService.setTaraweehToday(next);
    _loadFastingState();
  }
              ...
              : ListView(padding: EdgeInsets.fromLTRB(20, 20, 20, 20 + MediaQuery.of(context).padding.bottom),
                  children: [
                    ...
                    if (isRamadan) ...[
                      ...
                      Card(
                        child: SwitchListTile(
                          title: Text(Localizations.localeOf(context).languageCode == 'ar' ? 'صليت التراويح الليلة' : 'Prayed Taraweeh tonight'),
                          subtitle: Text(Localizations.localeOf(context).languageCode == 'ar' ? 'سجّل صلاة التراويح اليومية' : 'Log your nightly Taraweeh prayer'),
                          value: _taraweehToday,
                          activeTrackColor: AppColors.primaryEmerald,
                          onChanged: (_) => _toggleTaraweeh(),
                        ),
                      ),
                      const SizedBox(height: 12),
                      Card(
